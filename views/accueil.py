...
def get_dashboard_stats(school_id, is_super_admin):
    ...
    try:
        ...
        q_profs_table = db.query(Enseignant)
        ...
        if not is_super_admin and school_id:
            ...
            q_profs_table = q_profs_table.filter(Enseignant.school_id == school_id)
        ...
        if total_profs_db > 0:
            ...
            total_profs = (
                q_users.filter(
                    or_(User.role == "prof", User.role == "enseignant")
                ).count()
    ...
    with col_a1:
        ...
        try:
            ...
            if is_super_admin:
                ...
                user_role = (
                    getattr(user_obj, "role", "").lower() if user_obj else ""
                )
                is_school_admin = user_role in [
                    "admin",
                    "directeur",
                    "proviseur",
                    "censeur",
            ...
            derniers_logs = (
                q_logs.order_by(desc(JournalActivite.timestamp)).limit(5).all()
            )

            if derniers_logs:
                for log in derniers_logs:
                    heure = (
                        log.timestamp.strftime("%H:%M") if log.timestamp else "N/D"
                    )
                    couleur_statut = (
                        "#EF4444"
                        if getattr(log, "statut", "") in ["Critique", "Avertissement"]
                        else "#10B981"
                    )
