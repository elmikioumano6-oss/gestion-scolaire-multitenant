        ...
        if st.button("🗂️ Générer l'archive ZIP globale de l'établissement", type="secondary"):
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                ...
                mapping_tables = {
                    ...
                    "paiements": (Paiement, ["reference_recu", "eleve_id", "montant", "mode_reglement", "motif", "nom_payeur", "date_paiement"]),
                    "dépenses": (Depense, ["libelle", "montant", "categorie", "cycle", "auteur", "date_depense"]),
                    "enseignants": (Enseignant, ["nom", "prenom", "specialite", "statut", "telephone"]),
                    "notes": (Note, ["eleve_id", "matiere_id", "valeur", "type_evaluation", "semestre"])
                }

                for nom_t, (model_t, cols) in mapping_tables.items():
                    q_z = db.query(model_t)
                    if not is_super_admin and school_id:
                        q_z = q_z.filter(model_t.school_id == school_id)
                    records_z = q_z.all()
            ...
            log_action_erp(
                module="Backup & Export",
                action=f"Génération d'une sauvegarde Snapshot ZIP globale pour l'établissement {school_name}",
                statut="Succès",
                valeur_avant="Tables opérationnelles courantes",
                valeur_apres="Archive ZIP globale générée avec succès"
            )

            st.success("✅ Archive de sauvegarde globale générée avec succès !")
            st.download_button(
                label="📥 Télécharger l'archive ZIP globale (.zip)",
                data=zip_buffer,
                file_name=f"backup_global_{school_name.replace(' ', '_').lower()}_{datetime.now().strftime('%Y%m%d')}.zip",
