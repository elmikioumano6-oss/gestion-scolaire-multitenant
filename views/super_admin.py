...
def afficher_super_admin():
    ...
    try:
        ...
        with st.expander(
            ...
            st.markdown(
                "Si vous avez des comptes administrateurs ou censeurs créés avant"
                " la mise en place de la sécurité, forcez ici l'exigence d'un"
                " changement de mot de passe à leur prochaine connexion."
            )
            if st.button(
            ...
            if not ecoles:
                ...
                for ecole in ecoles:
                    ...
                    nb_eleves_ecole = (
                        db.query(Eleve)
                        .filter(
                            Eleve.school_id == ecole.id, Eleve.deleted_at.is_(None)
                        )
                        .count()
                    )

                    with st.expander(
                        f"🏫 {ecole.nom} (ID: {ecole.id}) [Code: {ecole.code}] — Statut :"
                        ...
                        if current_active_school == ecole.id:
                            st.success(
                                "🟢 Cet établissement est actuellement actif dans votre"
                                " session de travail."
                            )
                        else:
                            if st.button(
                                ...
                                if submitted_update:
                                    ...
                                    if ecole_maj:
                                        ...
                                        log_action_erp(
                                            ...
                                            action=(
                                                f"Mise à jour des droits pour l'établissement"
                                                f" {ecole_maj.nom}"
                                            ),
                                            statut=(
                                    ...
                                    if existing_usr:
                                        if existing_usr.school_id == ecole.id:
                                            ...
                                            log_action_erp(
                                                ...
                                                valeur_apres=(
                                                    "Nouveau mot de passe provisoire exigé"
                                                ),
                                            )
                                            db.commit()
            ...
            with st.form("form_create_school"):
                ...
                if submitted_school:
                    if not nom_ecole.strip() or not code_ecole.strip():
                        ...
                        if code_existant:
                            st.error(
                                f"⚠️ Un établissement avec le code '{code_nettoye}' existe"
                                " déjà."
                            )
                        else:
                            user_existant = (
                            ...
                            if user_existant:
                                st.error(
                                    f"⚠️ L'identifiant '{admin_username.strip()}' est déjà"
                                    " utilisé."
                                )
                            else:
