...
from database.models import Classe, Eleve, Paiement
...
def afficher_eleves(niveau_actif="Collège"):
    ...
    st.markdown(
        "Enregistrement et suivi des effectifs scolaires avec isolation"
        " multi-tenant stricte et traçabilité ERP."
    )
    st.markdown("---")
    ...
    try:
        ...
        with tab_liste:
            st.markdown(
                f"### Effectifs Enregistrés — Établissement ({niveau_actif})"
            )
            ...
            if eleves:
                ...
                garcons = sum(
                    1
                    for e in eleves
                    if getattr(e, "sexe", "").lower()
                    in ["masculin", "m", "garçon"]
                )
                filles = sum(
                    1
                    for e in eleves
            ...
            if not eleves:
                ...
                # Ajout d'une colonne pour le téléphone dans l'en-tête
                ...
                for e in eleves:
                    montant_red = (
                        getattr(e, "montant_reduction", 0.0) or 0.0
                    )
                    red_str = (
                        f"{montant_red:,.0f}".replace(",", " ")
                        if montant_red > 0
                        else "0"
                    )
                    ...
                    with btn_col1:
                        if st.button(
                            "✏️",
                            key=f"edit_eleve_{e.id}",
                            help="Modifier cet élève",
                        ):
                            st.session_state[f"editing_eleve_{e.id}"] = True
                    with btn_col2:
                        if st.button(
                            "🗑️",
                            key=f"del_eleve_{e.id}",
                            help="Archiver (Soft Delete) cet élève",
                        ):
                    ...
                    if st.session_state.get(f"deleting_eleve_{e.id}", False):
                        st.warning(
                            f"Voulez-vous vraiment archiver l'élève **{e.nom}"
                            f" {e.prenom}** ?"
                        )
                        ...
                        with c_del1:
                            if st.button(
                                ...
                                log_action_erp(
                                    ...
                                    action=(
                                        "Archivage (Soft Delete) de l'élève"
                                        f" {e.nom} {e.prenom} (Mat:"
                                        f" {e.matricule})"
                                    ),
                                    statut="Critique",
                                    valeur_avant="Actif",
                                ...
                                st.session_state[
                                    f"deleting_eleve_{e.id}"
                                ] = False
                                st.rerun()
                        with c_del2:
                            if st.button(
                                "Annuler", key=f"canc_del_el_{e.id}"
                            ):
                                st.session_state[
                                    f"deleting_eleve_{e.id}"
                                ] = False
                    ...
                    if st.session_state.get(f"editing_eleve_{e.id}", False):
                        with st.form(key=f"form_edit_eleve_{e.id}"):
                            st.markdown(
                                f"**Modifier l'élève : {e.nom} {e.prenom}**"
                            )
                            ...
                            options_classes = {
                                cl.libelle: cl.id for cl in classes_dispo
                            }
                            current_classe_name = (
                                e.classe.libelle
                                if e.classe
                                and e.classe.libelle in options_classes
                                else list(options_classes.keys())[0]
                                if options_classes
                                else ""
                            )

                            new_matricule = st.text_input(
                                "Matricule", value=e.matricule or ""
                            )
                            new_nom = st.text_input("Nom", value=e.nom or "")
                            new_prenom = st.text_input(
                                "Prénom", value=e.prenom or ""
                            )
                            new_tuteur = st.text_input(
                                "Téléphone Parent / Tuteur (WhatsApp)",
                                value=e.tuteur or "",
                            )

                            sexes = ["Masculin", "Féminin"]
                            idx_sexe = (
                                sexes.index(e.sexe) if e.sexe in sexes else 0
                            )
                            new_sexe = st.selectbox(
                                "Sexe", sexes, index=idx_sexe
                            )

                            class_names = list(options_classes.keys())
                            idx_cls = (
                                class_names.index(current_classe_name)
                                if current_classe_name in class_names
                                else 0
                            )
                            new_classe_nom = st.selectbox(
                                "Classe", class_names, index=idx_cls
                            )
                            ...
                            idx_red = (
                                types_red.index(e.type_reduction)
                                if e.type_reduction in types_red
                                else 0
                            )
                            new_type_red = st.selectbox(
                                "Type de réduction", types_red, index=idx_red
                            )

                            new_montant_red = st.number_input(
                                "Montant de la réduction (FCFA)",
                                value=float(e.montant_reduction or 0.0),
                                step=5000.0,
                            )

                            sub_edit = st.form_submit_button(
                            ...
                            if sub_edit:
                                if (
                                    not new_nom
                                    or not new_prenom
                                    or not new_matricule
                                ):
                                    st.error(
                                        "Le nom, le prénom et le matricule sont"
                                        " obligatoires."
                                    )
                                else:
                                    ancienne_valeurs = (
                                        f"Nom: {e.nom}, Prénom: {e.prenom},"
                                        f" Mat: {e.matricule}"
                                    )
                                    ...
                                    st.success(
                                        "Informations de l'élève modifiées et"
                                        " tracées avec succès !"
                                    )
                                    st.session_state[
                                        f"editing_eleve_{e.id}"
                                    ] = False
                                    st.rerun()
                            if canc_edit:
                                st.session_state[
        ...
        with tab_ajout:
            ...
            if not classes_dispo:
                st.warning(
                    "⚠️ Veuillez d'abord créer des classes pour le cycle"
                    f" **{niveau_actif}** dans le menu 'Classes & Tarifs'."
                )
                return
            ...
            with st.form("form_inscription_eleve"):
                ...
                if classe_obj_selectionnee:
                    scol_ref = (
                        f"{classe_obj_selectionnee.frais_scolarite:,.0f}"
                        .replace(",", " ")
                    )
                    insc_ref = (
                        f"{classe_obj_selectionnee.frais_inscription:,.0f}"
                        .replace(",", " ")
                    )
                    coges_ref = (
                        f"{getattr(classe_obj_selectionnee, 'frais_coges', 0.0):,.0f}"
                        .replace(",", " ")
                    )
                    trans_ref = (
                        f"{classe_obj_selectionnee.frais_transport:,.0f}"
                        .replace(",", " ")
                    )
                    cant_ref = (
                        f"{classe_obj_selectionnee.frais_cantine:,.0f}"
                        .replace(",", " ")
                    )

                    st.info(
                        f"📌 **Grille tarifaire officielle de la classe"
                        f" ({classe_choisie_nom})** :\n- Scolarité :"
                        f" **{scol_ref} F** | Inscription : **{insc_ref} F** |"
                        f" COGES : **{coges_ref} F**\n- Transport : **{trans_ref}"
                        f" F** | Cantine : **{cant_ref} F**"
                    )

                st.markdown(
                    "#### 💰 Ventilation des Versements Initiaux à l'Inscription"
                    " (FCFA)"
                )
                ...
                with col_v1:
                    versement_inscription = st.number_input(
                        ...
                        value=(
                            float(
                                classe_obj_selectionnee.frais_inscription
                                or 0.0
                            )
                            if classe_obj_selectionnee
                            else 0.0
                    ...
                    versement_coges = st.number_input(
                        ...
                        value=(
                            float(
                                getattr(
                                    classe_obj_selectionnee, "frais_coges", 0.0
                                )
                            )
                            if classe_obj_selectionnee
                            else 0.0
                ...
                with col_v2:
                    versement_scolarite = st.number_input(
                        "Acompte Scolarité (1ère Tranche)",
                        min_value=0.0,
                        value=0.0,
                        step=5000.0,
                    )
                    versement_transport = st.number_input(
                        "Part Transport",
                        min_value=0.0,
                        value=0.0,
                        step=1000.0,
                ...
                with col_v3:
                    versement_cantine = st.number_input(
                        "Part Cantine",
                        min_value=0.0,
                        value=0.0,
                        step=1000.0,
                ...
                st.markdown(
                    "#### 🏷️ Réduction sur les Frais de Scolarité (Optionnel)"
                )
                col_red1, col_red2 = st.columns(2)
                with col_red1:
                    type_reduction = st.selectbox(
                        "Type de réduction",
                        [
                            "Aucune",
                            "Bourse scolaire",
                ...
                submitted = st.form_submit_button(
                    "Valider l'inscription & encaisser", type="primary"
                )
                if submitted:
                    if not nom_e or not prenom_e or not matricule_e:
                        st.error(
                            "Le nom, le prénom et le matricule de l'élève sont"
                            " obligatoires."
                        )
                    else:
                        ...
                        if doublon_mat:
                            st.error(
                                f"⚠️ Un élève avec le matricule"
                                f" '{matricule_e.strip()}' existe déjà dans cet"
                                " établissement."
                            )
                        else:
                            ...
                            nouvel_eleve = Eleve(
                                ...
                                tuteur=(
                                    tuteur_e.strip() if tuteur_e else None
                                ),
                                sexe=sexe_e,
                                cycle=niveau_actif,
                            ...
                            for (
                                ...
                                if montant_verse > 0:
                                    ...
                                    nouveau_paiement = Paiement(
                                        ...
                                        agent_caisse=st.session_state.get(
                                            "username", "admin"
                                        ),
                            ...
                            log_action_erp(
                                ...
                                action=(
                                    ...
                                    f" {nom_e.upper()} {prenom_e} (Mat:"
                                    f" {matricule_e.strip()}) - Total encaissé:"
                                    f" {total_encaisse:,.0f} F"
                                ),
                                statut="Succès",
                                valeur_avant="Inexistant",
                                valeur_apres=(
                                    f"Inscrit en {classe_choisie_nom} avec"
                                    f" {total_encaisse:,.0f} F de versements"
                                    " initiaux"
                                ),
                            )

                            st.success(
                                f"Élève **{nom_e} {prenom_e}** inscrit avec"
                                f" succès ! Total encaissé à la caisse : **{total_encaisse:,.0f} FCFA**."
                            )
                            st.rerun()
    finally:
        db.close()
